...
contract FlareETHFeed {
    ...
    function main() external {
        ...
        if (prices.length > buyLookbackPeriods + 1) {
    ...
    function executeStrategy() internal {
        ...
        uint256 currentPrice = prices[prices.length - 1].price;

        // Buy signal: current price > all last buyLookbackPeriods
        if (!hasPosition && shouldBuy(currentPrice)) {
            buyETH();
        }

        // Sell signal: current price < all last sellLookbackPeriods
        if (hasPosition && shouldSell(currentPrice)) {
            sellETH();
        }
    }

    function shouldBuy(uint256 currentPrice) internal view returns (bool) {
    ...
    function shouldSell(uint256 currentPrice) internal view returns (bool) {
    ...
    function getPosition() external view returns (uint256 ethAmount, bool hasPos) {
        return (ethPosition, hasPosition);
    }
}
